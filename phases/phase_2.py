...
def agent_blueprint(system_prompt, history, last_messages, next_agent: list):
    ...
    return{"messages":llm_response, "next_agent":checked.next_agent, }



def kouhei(state: State):
    Kouhei_phase = Kouhei_prompt(phase_3_prompt)
...
def susumu(state: State):
    Kouhei_phase = susumu_prompt(phase_3_prompt)
    history = state["summerized"]
    last_messages = state["messages"]
...
def masami(state: State):
    Kouhei_phase = masami_prompt(phase_3_prompt)
    history = state["summerized"]
    last_messages = state["messages"]
...
def start(state: State):
    ...
    last_agent = ""
    next_agent = "Kouhei"
    messages = "We are moving to phase 2. we are analyzing the clues. Kouhei can you go ahead?"
